...
public class Location extends CachedData {
   ...
   *
   * @param owner  location owner
   * @param time   time at which the location must be live
   * @param itemID item ID for which the location is being requested
  ...
  public static Location get(
    ...
    try {
      return EveKitUserAccountProvider.getFactory()
                                      .runTransaction(() -> {
                                        TypedQuery<Location> getter = EveKitUserAccountProvider.getFactory()
                                                                                               .getEntityManager()
                                                                                               .createNamedQuery(
                                                                                                   "Location.getByItemID",
                                                                                                   Location.class);
   ...
   * @param time       time at which contact must be live
   * @param maxresults maximum number of contacts to retrieve
   * @param contid     sortKey (exclusive) from which to start returning results.
    ...
    try {
      return EveKitUserAccountProvider.getFactory()
                                      .runTransaction(() -> {
                                        ...
                                        // Return result
                                        TypedQuery<Location> query = EveKitUserAccountProvider.getFactory()
                                                                                              .getEntityManager()
                                                                                              .createQuery(
                                                                                                  qs.toString(),
                                                                                                  Location.class);
