...
@Entity
@Table(name = "evekit_data_jump_clone_implant", indexes = {
    @Index(name = "jumpCloneIDIndex", columnList = "jumpCloneID", unique = false), @Index(name = "typeIDIndex", columnList = "typeID", unique = false),
})
@NamedQueries({
    @NamedQuery(
...
public class JumpCloneImplant extends CachedData {
  ...
  public static JumpCloneImplant get(final SynchronizedEveAccount owner, final long time, final int jumpCloneID, final int typeID) {
    try {
  ...
  public static List<JumpCloneImplant> getAll(final SynchronizedEveAccount owner, final long time) {
    try {
